    ...
    <main style={{padding:24}}>
      <section style={card}>
        <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10}}>
          <div><h1 style={{margin:'0 0 6px'}}>{data.name}</h1><div style={{opacity:.8}}>{data.trade} • {data.city}</div></div>
      ...
      <section style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        <div style={card}><h3>Services</h3>
          <div>{services.length?services.map((s,i)=>(<div key={i}>• {s}</div>)):<p style={{opacity:.7}}>Add your services</p>}</div></div>
        <div style={card}><h3>Hours & Contact</h3>
