...
export default function PublicPage(){
  ...
  const mailHref = null; // no public email in schema – button hides automatically
    ...
    <div style={pageWrap}>
      ...
      <div style={topBar}>
        <div style={{display:'flex',alignItems:'center',gap:12}}>
          ...
          <div>
            <div style={{fontWeight:700}}>{p.name || p.slug}</div>
            <div style={{opacity:.7,fontSize:14}}>{[p.trade, p.city].filter(Boolean).join(' • ')}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:16,flexWrap:'wrap'}}>
        <h1 style={{margin:'0 0 10px 0',fontSize:36}}>{p.name || p.slug}</h1>
        <div style={{display:'flex',gap:10}}>
          {callHref && <a href={callHref} style={{...chipBtn, ...primaryBtn}}>Call</a>}
          {waHref && <a href={waHref} style={chipBtn}>WhatsApp</a>}
          {mailHref && <a href={mailHref} style={chipBtn}>Email</a>}
        </div>
      </div>
      <div style={{opacity:.7,marginTop:-4,marginBottom:16}}>{[p.trade, p.city].filter(Boolean).join(' • ')}</div>
      ...
      <div style={grid2}>
        ...
        <Card title="About">
          <p style={{marginTop:0,marginBottom:14}}>
            {/* You can change this sentence anytime; for now we show a short friendly blurb. */}
            {services[0] ? `${services[0]}. Reliable, friendly and affordable. Free quotes, no hidden fees.` : 'Reliable, friendly and affordable. Free quotes, no hidden fees.'}
          </p>
          {areas.length>0 && (
...
const logoDot = {
  ...
  background:'#0e2037', border:'1px solid #1a355d', color:'#8bd0ff', fontWeight:700
}
...
const ulBullets = {
  margin:0, paddingLeft:20, display:'grid', gap:6
}
