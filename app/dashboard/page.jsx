...
export default function Dashboard(){
  ...
  useEffect(()=>{ if(!user) return; supabase.from('pages').select('*').eq('owner',user.id).limit(1).single().then(({data})=>{ if(data){setPage(data);setForm({...form,...data})} }) },[user])
  ...
  const createPage = async ()=>{
    ...
    setMsg(error?error.message:'Created!'); if(!error) setPage(data)
  }
